...
function startUnityStreaming()
    ...
    % Camera setup - AUTO-DETECT FRAMERATE PROPERTY
    imaqreset;
    ...
    % Try to set framerate using common property names
    frameRateSet = false;
    possibleProps = {'FrameRate', 'AcquisitionFrameRate', 'AcquisitionFrameRateAbs', ...
                     'FrameRatePercentage', 'Framerate', 'CameraFrameRate'};
    
    fprintf('Detecting framerate property...\n');
    for i = 1:length(possibleProps)
        try
            propInfo = propinfo(src, possibleProps{i});
            % Property exists, try to set it
            if isfield(propInfo, 'ConstraintValue')
                % Find closest to 60 fps
                availableRates = propInfo.ConstraintValue;
                fprintf('  Found %s with values: %s\n', possibleProps{i}, strjoin(availableRates, ', '));
                
                % Try to set to 60 fps or closest
                if ismember('60.0000', availableRates)
                    set(src, possibleProps{i}, '60.0000');
                    currentRate = '60.0000';
                elseif ismember('60.0002', availableRates)
                    set(src, possibleProps{i}, '60.0002');
                    currentRate = '60.0002';
                elseif ismember('59.9940', availableRates)
                    set(src, possibleProps{i}, '59.9940');
                    currentRate = '59.9940';
                else
                    % Use the highest available
                    set(src, possibleProps{i}, availableRates{end});
                    currentRate = availableRates{end};
                end
                
                fprintf('✓ Camera framerate set to: %s using property: %s\n', currentRate, possibleProps{i});
                frameRateSet = true;
                break;
            end
        catch
    ...
    if ~frameRateSet
        fprintf('⚠ WARNING: Could not set framerate. Using camera default.\n');
        fprintf('  Run checkCameraProperties() to see available properties.\n');
    end
        ...
        try
            ...
            while running && ishandle(f)
                ...
                if isrunning(vid) && vid.FramesAvailable > 0
                    ...
                    % GPU-accelerated RGB processing
                    ...
                    % Update display
                    if params.showPreview && toc(lastDisplayUpdate) > 0.033
