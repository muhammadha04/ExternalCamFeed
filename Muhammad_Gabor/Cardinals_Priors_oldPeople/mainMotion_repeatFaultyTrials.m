...
Screen('CloseAll'); 
motion_params_estiDisc_testing_Disc_3angs_pilot_B10; global params;  
...
for b = 1:params.blockVars.numBlocks
    ...
    for t = 1: params.trialVars.numTrialsPerBlock
        ...
        if params.eye.record
         ...
         [respDiscTrial, respEstiTrial, dircs, allPosPix, faultyTrial] = trial(wPtr, expData.angles(trialNum),startclut,direc,edfFile, sesNum, trialNum, el, expData.trialsType(trialNum),expData.dotCoh(trialNum));
         faultyTrial
         if ~faultyTrial,
             ...
             fprintf("faulty trial save ..")
             faultyTrialsNum
             faultyTrialsNum = [faultyTrialsNum,trialNum];
         end
         save(saveExpFile)
    end
    
%     t=0;
%     for t = 1: faultyTrialsNum
%             fprintf("running faulty trials..")
%             faultyTrialsNum
%             t
%             trialNum = faultyTrialsNum(t)
%             edfFile = sprintf('%st%d.edf',initials,trialNum); el = NaN;
%             if params.eye.record
%                 edfFileStatus = Eyelink('Openfile', edfFile);
%                 if edfFileStatus ~= 0, fprintf('Cannot open .edf file. Exiting ...'); return; end       
