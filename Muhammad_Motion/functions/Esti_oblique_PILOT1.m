...
%      stimuli params 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
stim = struct('dur', {.85}, 'possibleAngels', {[310 330]},'boundaryAngle', {[320 320]},...
...
stim.cw_ccw = [1 2 1 2 1 2 ]; %CW (1) or CCW (2) from the boundary, for example 3deg is CCW from 0deg
...
%      Stair params 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
stair = struct( 'runType',{0},'numStaircases', {3}, 'numDownStep',{[2 2 2]},...
...
mouse = struct( 'color',{[black black black 255]},'penWidthPix', {1}, 'radiusDeg', {5},...
                'present',{1}, 'rectDeg', {0.15},'initialPosVar', {25}, 'respWin',{10}); 
...
responseVar = struct( 'allowedRespKeys', {['6', '3']}, 'dur',{30},'keyEscape', 'ESCAPE', 'percentEstimation', {1});
...
%      Save Data params
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
saveVars = struct('fileName', {'motionExp_testing_disc_'}, 'expTypeDirName', {'Revised_Exp'});
